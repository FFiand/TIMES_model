...
$ifThenI.solvemode %SOLVEMODE%==SOLVE
  ...
  file modelstatus / '%out_dir%%system.DirSep%%MODEL_NAME%_modelstat.inc' / ;
  put modelstatus '%MODEL_NAME%.modelstat = ' %MODEL_NAME%.modelstat:0:0 ';' ; 
...
* load solution file and fake modelstatus from "real" solve  
  execute_loadpoint '%out_dir%%system.DirSep%%MODEL_NAME%_p.gdx';
  $$include %out_dir%%system.DirSep%%MODEL_NAME%_modelstat.inc  
$endif.solvemode 
