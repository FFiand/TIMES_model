...
$   BATINCLUDE equserco.mod  mod %1 "RT(T,R,UC_N%SWT%)$UC_RHSMAP(UC_N,'EACH','SEVERAL',R,T,'ANNUAL'%3)"                                                         "("        "("             "SUM(UC_TS_SUM(R,UC_N,S),"   ""                      0    %VAR%_UCRT(UC_N,R,T%SOW%)     UC_RHSRT(R,UC_N,T,%2)
